      ...
      <div className="content">
        ...
        { this.recommendations.length === 0 &&
          <div className="recommendation">
            ...
            <p>Il n'y a malheureusement pas (encore) de liste citoyenne (qui n'émane pas des partis politiques existants) dans votre commune qui milite non pas pour un programme mais avant tout pour une nouvelle façon d'inclure tous les citoyens dans les prises de décisions politiques.</p>
...
const ListSummary = ({ key, list, city }) => (
  <div key={key} className="ListSummmary">
    ...
    <h3 className="listname">
      <Link href={`/villes/${city.name}/${list.name}`}><a>{list.name}</a></Link>
    </h3>
    <ListWarnings list={list} />
    <div className="stats">
      <span className="col"><Link href={`/villes/${city.name}/${list.name}`}><a>{list.candidates.length} candidats</a></Link></span>
      { list.totalPoliticians ? <span className="col"> dont au moins {list.totalPoliticians} {list.totalPoliticians > 1 ? 'sont des politiciens' : 'est un.e politicien.ne'}</span> : '' }
    { (list.totalCumuls || list.totalYearsInPolitics)
        ? <div className="col">Ensemble, ils cumulent plus de {list.totalCumuls} mandats et ont déjà passé plus de {list.totalYearsInPolitics} années en politique.</div>
        : '' }
    </div>
  </div>
)
